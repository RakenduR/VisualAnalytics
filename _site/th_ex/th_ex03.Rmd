...
```{r}
packages = c('tidyverse','ggdist','gghalves','ggthemes','hrbrthemes','ggridges','patchwork','zoo', 'ggrepel','ggiraph','lubridate')
for(p in packages){
...
In the below code chunk, financial data of participants are grouped by Month and Aggregated to find their monthly Income and Expenses. The data is then joined with Participants data so that we have information about participants education level and other details.
...
p1 <- participant_fin %>%
  ggplot(aes(x=date, y = income, group =participantId, color =educationLevel))+
  geom_line_interactive(aes(tooltip = tooltip),size =0.3)+
  ylab("Income")+
  xlab("Month, Year")+
...
From the above plot, we can see that the expenses are more or less same for different education levels and there is not much variation. This means that the savings will be proportional to the income and we will analyse this further down the line. Now we will continue to analyse the monthly income variations of different education levels using cyclic graph.
...
* Financial health of the occupants have deteriorated between 2022 and 2023
